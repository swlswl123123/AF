...
ifft_length = 2^nextpow2(Num_carrier);%IFFT点数
...
carrier_data = [carrier_data(1:Num_carrier/2,:);zeros(1,Num_symbol_data);carrier_data(Num_carrier/2+1:end,:)]; %插入直流子载波
carrier_data_temp1 = zeros(ifft_length,Num_symbol_data);
carrier_data_temp1(ifft_length/2-Num_carrier/2+1:ifft_length/2+Num_carrier/2+1,:) = carrier_data;%添加保护带宽
